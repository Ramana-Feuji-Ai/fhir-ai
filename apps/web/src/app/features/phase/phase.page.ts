  ...
  template: `
    @if (phase(); as p) {
      <section class="phaseHero">
        <div class="wrap">
          <a class="back" routerLink="/" fragment="curriculum">← Back to curriculum</a>
          <div class="phaseGrid">
            <div>
              <div class="chips">
                ...
                @if (p.resources.lab) {
                  <span class="chip orange">Lab Available</span>
                }
                      ...
                      @if (p.resources.lab) {
                        <div class="resourceSection labSection">
                          ...
                          <div class="labCard">
                            ...
                            <button class="btn primary" (click)="openLab()">Launch Lab</button>
                          </div>
                        </div>
                      }
                      @if (p.resources.quiz.length) {
              ...
              @if (!collapsed()[si]) {
                <ul class="topics">
                  @for (t of sec.topics; track t.id) {
                    <li class="topicRow" [class.open]="openTopicId() === t.id" [class.completed]="t.completed">
                      <div class="topicHead">
                        <button
                          type="button"
                          class="check"
                      ...
                      @if (openTopicId() === t.id && topicDetail(); as detail) {
...
export class PhasePage implements OnInit, OnDestroy {
  ...
  /** SCORM "Done" on last slide → mark topic completed. */
  onSlideDeckCompleted(topicId: number): void {
    this.toggleComplete(topicId, true);
  }
  ...
  openLab(): void {
    alert('Lab launching... In a real implementation, this would open a FHIR sandbox environment.');
  }
